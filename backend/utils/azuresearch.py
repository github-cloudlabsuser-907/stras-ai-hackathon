...
AZURESEARCH_DIMENSIONS = int(
    os.environ.get("AZURESEARCH_DIMENSIONS", 1536))  # Default to OpenAI's ada-002 embedding model vector size
...
def get_search_client(endpoint: str, key: str, index_name: str,
                      semantic_configuration_name: str = None) -> SearchClient:
...
class AzureSearch(VectorStore, ABC):
    def __init__(
            self,
        ...
        try:
            ...
            raise ValueError(
                ...
                "Please install it with `pip install --index-url "
        ...
        self.client = get_search_client(
            self.azure_cognitive_search_name, self.azure_cognitive_search_key, self.index_name,
...
class AzureSearchVectorStoreRetriever(BaseRetriever, BaseModel, ABC):
    ...
    @staticmethod
    def validate_search_type(values: Dict) -> Dict:
